...
class MaxThreeOfAKind(Env):
    ...
    def step(self, action):
        ...
        for i in self.dicevalues:
            if self.dicevalues.count(i) >= 3:
                return observation, sum(self.dicevalues)/30, True, info
        else:
...
def golearn(allrolls):
    ...
    log_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "LogThreeOfAKind")
    ppo_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "Models", "PPO_MaxThreeOfAKind")


    env = MaxThreeOfAKind(allrolls)
    env = Monitor(env)
