...
class MaxSmallStreet(Env):
    ...
    def step(self, action):
        ...
        info = {}
        for x in [1, 2, 3]:
            if x in self.dicevalues and x + 1 in self.dicevalues and x + 2 in self.dicevalues and x + 3 in self.dicevalues:
...
def golearn(allrolls):
    log_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "LogSmallStreet")
    ppo_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "Models", "PPO_MaxSmallStreet")


    env = MaxSmallStreet(allrolls)
    env = Monitor(env)
