...
class MaxThrees(Env):
    ...
    def step(self, action):
        ...
        threecount = self.dicevalues.count(3)
        observation = {"rolls":self.allrolls.index(tuple(self.dicevalues))}
        reward = threecount if threecount > 0 else -1
...
def golearn(allrolls):
    ...
    log_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "LogThrees")
    ppo_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "Models", "PPO_MaxThrees")


    env = MaxThrees(allrolls)
    env = Monitor(env)
