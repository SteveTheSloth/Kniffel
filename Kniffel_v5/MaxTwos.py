...
class MaxTwos(VectorEnv):
    ...
    def step(self, action):
        ...
        twocount = self.dicevalues.count(2)
        observation = {"rolls":self.allrolls.index(tuple(self.dicevalues))}
        reward = twocount if twocount > 0 else -1
...
def golearn(allrolls):
    log_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "LogTwos")
    ppo_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "Models", "PPO_MaxTwos")


    env = MaxTwos(allrolls, num_envs=4)
    env = Monitor(env)
