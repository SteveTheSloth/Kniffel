...
class MaxSixes(Env):
    ...
    def step(self, action):
        ...
        sixcount = self.dicevalues.count(6)
        observation = {"rolls":self.allrolls.index(tuple(self.dicevalues))}
        reward = sixcount if sixcount > 0 else -1
...
def golearn(allrolls):
    log_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "LogSixes")
    ppo_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "Models", "PPO_MaxSixes")


    env = MaxSixes(allrolls)
    env = Monitor(env)
