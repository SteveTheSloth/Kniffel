...
class MaxFives(Env):
    ...
    def step(self, action):
        ...
        fivecount = self.dicevalues.count(5)
        observation = {"rolls":self.allrolls.index(tuple(self.dicevalues))}
        reward = fivecount if fivecount > 0 else -1
...
def golearn(allrolls):
    log_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "LogFives")
    ppo_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "Models", "PPO_MaxFives")


    env = MaxFives(allrolls)
    env = Monitor(env)
