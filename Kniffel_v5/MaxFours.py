...
class MaxFours(Env):
    ...
    def step(self, action):
        ...
        fourcount = self.dicevalues.count(4)
        observation = {"rolls":self.allrolls.index(tuple(self.dicevalues))}
        reward = fourcount if fourcount > 0 else -1
...
def golearn(allrolls):
    ...
    log_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "LogFours")
    ppo_path = os.path.join("RL_Kniffel", "Kniffel_PPO", "Models", "PPO_MaxFours")


    env = MaxFours(allrolls)
    env = Monitor(env)
